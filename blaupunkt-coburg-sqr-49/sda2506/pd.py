...
#ann_cmdbit, ann_databit, ann_read, ann_write, ann_erase, ann_warning = range(6)
...
class Decoder(srd.Decoder):
    ...
    def decode_field(self, name, offset, width):
        ...
        self.put(ss, es, self.out_ann, [ann_data,
            ['%s: %02X' % (name, val)]])
        return val
    ...
    def decode(self):
        ...
        while True:
            clk, d, ce = self.wait([{0: 'e'}, {2: 'e'}]) # wait for CLK edge or CE edge
            if self.matched[0] and ce == 1 and clk == 1: #raising clk edge and command mode
                bitstart = self.samplenum
                self.wait({0: 'f'})
                self.cmdbits = [(d,bitstart,self.samplenum)] + self.cmdbits
                if len(self.cmdbits) > 24: self.cmdbits = self.cmdbits[0:24]
                self.putbit(bitstart, self.samplenum, ann_cmdbit, d)

            elif self.matched[0] and ce == 0 and clk == 0: #falling clk edge and data mode
                bitstart = self.samplenum
                clk, d, ce = self.wait([{'skip': int(2.5 * (1e6 / self.samplerate))}, {0: 'r'}, {2: 'e'}]) # wait 25 us for data ready
                if self.matched == (True, False, False): self.wait([{0: 'r'}, {2: 'e'}])
            ...
            elif self.matched[1] and ce == 0: #chip enable edge
                try:
                    ...
                    if self.cmdbits[0][0] == 0:
                        ...
                        self.put(self.cmdbits[7][1], self.samplenum, self.out_ann, [ann_cmd,
                            ['read' ]])
                    elif d == 0:
                        ...
                        self.wait({2: 'r'})
                        self.put(cmdstart, self.samplenum, self.out_ann, [ann_cmd,
                            ['Write to %02X: %02X' % (addr, data)]])
                    else:
